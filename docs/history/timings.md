...
| 29-08-2026 | Post-launch, demos button | UI | 3 tasks | roughly 30 minutes | `DEMO_LINK` in `CTAButtons.jsx` set to `https://demo.rkade.co` once the separate Jewelry-Demo project deployed, hidden Demos button on `/links` turned on and relabelled, one preview correction to the sub-line so it stays one line on a phone. Includes the demo's own deploy and verification. No debug rounds. Branch `demos-button`, PR #14, merged `--no-ff` to `main` as 782d58f, verified live. |
